...
func (fetcher *FetchCmd) Fetch(results chan map[string]interface{}, errors chan error) {
	page := 0
	finished := false
...
func (fetcher *FetchCmd) readByPID() (map[string]interface{}, error) {
	...
	if user == nil {
		return nil, fmt.Errorf("failed to get user by pid %s", fetcher.UserPID)
	}
...
func (fetcher *FetchCmd) readByEmail() ([]map[string]interface{}, error) {
	...
	if len(auth0Users) < 1 {
		return nil, fmt.Errorf("failed to get user by email %s", fetcher.UserEmail)
	}
